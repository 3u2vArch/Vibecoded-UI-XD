--[[
	CoreUI — a self-contained Roblox Lua UI library (fixed build)
	----------------------------------------------------------------
	Fixes in this build:
	  • Sidebar UIListLayout no longer pushed down by UIPadding
	  • Notifications no longer stretch to full screen height
	  • Theme changes now re-tint existing widgets
	  • Lucide provider auto-detects API shape and falls back
	    through several URLs (.luau, .lua, jsDelivr mirror)
	  • Custom cursor uses a proper crosshair asset by default
	  • Window:SetStatus() added
	  • Enum.AutomaticCanvasSize → Enum.AutomaticSize everywhere
]]
...
function Util.Clamp(n, min, max) return math.clamp(n, min, max) end
function Util.IsMobile() return UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled end
...
function Util.Corner(radius) return New("UICorner", { CornerRadius = UDim.new(0, radius or 6) }) end
...
--============================================================
-- ICONS (Lucide) — multi-URL, multi-shape
--============================================================

local Icons = { Provider = nil, _shape = nil, _loaded = false }

local LUCIDE_URLS = {
	"https://raw.githubusercontent.com/latte-soft/lucide-roblox/master/lib/init.lua",
	"https://raw.githubusercontent.com/latte-soft/lucide-roblox/master/lib/init.luau",
	"https://cdn.jsdelivr.net/gh/latte-soft/lucide-roblox@master/lib/init.lua",
}
...
local function tryIcon(provider, shape, name, size, overrides)
	local ok, result = pcall(function()
		if shape == "ImageLabel" then return provider.ImageLabel(name, size, overrides)
		elseif shape == "Image"   then return provider.Image(name, size, overrides)
		elseif shape == "Get"     then return provider:Get(name, size, overrides)
		elseif shape == "getIcon" then return provider:getIcon(name, size, overrides)
		elseif shape == "Icon"    then return provider.Icon(name, size, overrides)
		end
	end)
...
function Icons.AutoLoad()
	if Icons._loaded then return true end
	local mod = tryLoadLucide()
	if mod then
		Icons.SetProvider(mod)
		return Icons._loaded
	end
	return false
end
...
Themes.Spotix = {
	Name = "Spotix", Font = Enum.Font.GothamMedium, FontBold = Enum.Font.GothamBold,
	Background = Color3.fromRGB(11, 15, 20), Sidebar = Color3.fromRGB(13, 17, 23),
	Topbar = Color3.fromRGB(13, 17, 23), Card = Color3.fromRGB(17, 21, 28),
	CardAlt = Color3.fromRGB(21, 26, 34), Border = Color3.fromRGB(31, 37, 46),
	Divider = Color3.fromRGB(28, 33, 41), Text = Color3.fromRGB(237, 240, 244),
	SubText = Color3.fromRGB(141, 150, 163), MutedText = Color3.fromRGB(96, 103, 114),
	Accent = Color3.fromRGB(34, 197, 94), AccentDark = Color3.fromRGB(21, 128, 61),
	AccentText = Color3.fromRGB(6, 20, 12), Danger = Color3.fromRGB(239, 68, 68),
	Warning = Color3.fromRGB(234, 179, 8), Info = Color3.fromRGB(56, 139, 253),
	ToggleOff = Color3.fromRGB(43, 49, 58), SliderRail = Color3.fromRGB(35, 41, 50),
}

Themes.EvilSpotix = {
	Name = "Evil Spotix", Font = Enum.Font.GothamMedium, FontBold = Enum.Font.GothamBold,
	Background = Color3.fromRGB(11, 15, 20), Sidebar = Color3.fromRGB(13, 17, 23),
	Topbar = Color3.fromRGB(13, 17, 23), Card = Color3.fromRGB(17, 21, 28),
	CardAlt = Color3.fromRGB(21, 26, 34), Border = Color3.fromRGB(31, 37, 46),
	Divider = Color3.fromRGB(28, 33, 41), Text = Color3.fromRGB(237, 240, 244),
	SubText = Color3.fromRGB(141, 150, 163), MutedText = Color3.fromRGB(96, 103, 114),
	Accent = Color3.fromRGB(239, 68, 68), AccentDark = Color3.fromRGB(153, 27, 27),
	AccentText = Color3.fromRGB(24, 6, 6), Danger = Color3.fromRGB(239, 68, 68),
	Warning = Color3.fromRGB(234, 179, 8), Info = Color3.fromRGB(56, 139, 253),
	ToggleOff = Color3.fromRGB(43, 49, 58), SliderRail = Color3.fromRGB(35, 41, 50),
}

Themes.Obsidian = {
	Name = "Obsidian", Font = Enum.Font.GothamMedium, FontBold = Enum.Font.GothamBold,
	Background = Color3.fromRGB(18, 18, 22), Sidebar = Color3.fromRGB(15, 15, 19),
	Topbar = Color3.fromRGB(15, 15, 19), Card = Color3.fromRGB(24, 24, 29),
	CardAlt = Color3.fromRGB(29, 29, 35), Border = Color3.fromRGB(40, 40, 48),
	Divider = Color3.fromRGB(35, 35, 42), Text = Color3.fromRGB(240, 240, 245),
	SubText = Color3.fromRGB(150, 150, 160), MutedText = Color3.fromRGB(100, 100, 110),
	Accent = Color3.fromRGB(124, 58, 237), AccentDark = Color3.fromRGB(88, 28, 200),
	AccentText = Color3.fromRGB(255, 255, 255), Danger = Color3.fromRGB(239, 68, 68),
	Warning = Color3.fromRGB(234, 179, 8), Info = Color3.fromRGB(56, 139, 253),
	ToggleOff = Color3.fromRGB(45, 45, 53), SliderRail = Color3.fromRGB(38, 38, 46),
}
...
local function track(conn) table.insert(CoreUI._Connections, conn); return conn end

function CoreUI:SetIconProvider(mod) Icons.SetProvider(mod) end
function CoreUI:GetIconProvider() return Icons.Provider, Icons._loaded end

function CoreUI:RegisterThemeListener(fn) table.insert(self._ThemeListeners, fn) end

function CoreUI:SetTheme(themeOrName)
	local theme = type(themeOrName) == "string" and Themes[themeOrName] or themeOrName
	if not theme then return end
	self.Theme = theme
	for _, fn in ipairs(self._ThemeListeners) do pcall(fn, theme) end
end

CoreUI.Themes = Themes

--============================================================
-- ROOT SCREENGUI + OVERLAY LAYERS
--============================================================

local ScreenGui = New("ScreenGui", {
	Name = "CoreUI_" .. tostring(math.random(100000, 999999)),
	ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
	DisplayOrder = 999, IgnoreGuiInset = true, Parent = GetRootParent(),
})
CoreUI.ScreenGui = ScreenGui

local GlobalScale = New("UIScale", { Scale = 1 })

local TooltipLayer = New("Frame", {
	Name = "TooltipLayer", BackgroundTransparency = 1,
	Size = UDim2.fromScale(1, 1), ZIndex = 10000, Parent = ScreenGui,
})

-- Notify layer: anchored top-right, auto-size, does NOT stretch vertically
local NotifyLayer = New("Frame", {
	Name = "NotifyLayer", BackgroundTransparency = 1,
...
local CursorLayer = New("ImageLabel", {
	Name = "CustomCursor", BackgroundTransparency = 1,
	Size = UDim2.fromOffset(22, 22),
	Image = "rbxassetid://11037777246", -- subtle crosshair
...
function CoreUI:SetDPIScale(scale) GlobalScale.Scale = scale end
...
function CoreUI:SetCustomCursor(enabled, imageId)
	...
	if enabled then
		...
		track(RunService.RenderStepped:Connect(function()
			if Mouse then CursorLayer.Position = UDim2.fromOffset(Mouse.X, Mouse.Y) end
		end))
	else
...
function CoreUI:AttachTooltip(target, text, isDisabledFn, disabledReason)
	...
	local function show()
		...
		local label = New("TextLabel", {
			...
			Size = UDim2.new(0, 0, 0, 28), ZIndex = 10001,
	...
	track(target.MouseEnter:Connect(function() hovering = true; show() end))
	track(target.MouseLeave:Connect(function() hovering = false; HideTooltip() end))
end

--============================================================
-- NOTIFICATIONS (fixed: compact, auto-height, no stretch)
--============================================================
...
function CoreUI:Notify(config)
	...
	-- Fixed-width, auto-height card. LayoutOrder keeps newest on top.
	...
	if config.Content then
		New("TextLabel", {
			BackgroundTransparency = 1,
			Text = config.Content,
			Font = theme.Font, TextSize = 13, TextWrapped = true,
...
function CoreUI:LoadConfig(name)
	...
	for flag, value in pairs(decoded) do
		...
		if element and element.Set then element:Set(value) end
...
function CoreUI:OnUnload(fn) table.insert(self._UnloadCallbacks, fn) end
...
local function BuildRow(parent, theme, opts)
	...
	New("UIListLayout", { Parent = textHolder, SortOrder = Enum.SortOrder.LayoutOrder, VerticalAlignment = Enum.VerticalAlignment.Center })
	...
	if not opts.Description then title.Size = UDim2.new(1, 0, 1, 0) end
...
local function CreateDivider(parent, theme)
	return New("Frame", { BackgroundColor3 = theme.Divider, Size = UDim2.new(1, 0, 0, 1), Parent = parent })
end
...
local function CreateWarningBox(parent, theme, opts)
	...
	local box = New("Frame", {
		BackgroundColor3 = color, BackgroundTransparency = 0.88,
		AutomaticSize = Enum.AutomaticSize.Y,
		Size = UDim2.new(1, 0, 0, 0), Parent = parent,
	}, { Util.Corner(8), Util.Stroke(color, 1, 0.5), Util.Pad(10, 12, 10, 12) })
	local holder = New("Frame", {
		BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y,
		Size = UDim2.new(1, 0, 0, 0), Parent = box,
	})
	New("UIListLayout", { Parent = holder, Padding = UDim.new(0, 2) })
	New("TextLabel", {
		BackgroundTransparency = 1, Text = "⚠ " .. (opts.Title or kind),
		Font = theme.FontBold, TextSize = 13, TextColor3 = color,
		TextXAlignment = Enum.TextXAlignment.Left,
	...
	New("TextLabel", {
		...
		AutomaticSize = Enum.AutomaticSize.Y,
		Size = UDim2.new(1, 0, 0, 0), Parent = holder,
	})
	return { Instance = box }
end
...
local function CreateButton(parent, theme, opts)
	...
	local row, controlArea = BuildRow(parent, theme, {
		Title = opts.Title, Description = opts.Description, Icon = opts.Icon, ControlWidth = 90,
	})
	local btn = New("TextButton", {
		...
		Font = theme.FontBold, TextSize = 13, TextColor3 = theme.AccentText,
		AutoButtonColor = false, Parent = controlArea,
	}, { Util.Corner(6) })
	track(btn.MouseButton1Click:Connect(function()
		if opts.Callback then task.spawn(opts.Callback) end
	end))
	track(btn.MouseEnter:Connect(function() Util.Tween(btn, 0.12, { BackgroundColor3 = theme.AccentDark }) end))
	track(btn.MouseLeave:Connect(function() Util.Tween(btn, 0.12, { BackgroundColor3 = theme.Accent }) end))
...
local function CreateToggleBase(parent, theme, opts, square)
	...
	if square then
		switch = New("Frame", {
			...
			AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, 0, 0.5, 0),
			Size = UDim2.fromOffset(22, 22), Parent = controlArea,
		}, { Util.Corner(5), Util.Stroke(theme.Border, 1) })
		knob = New("TextLabel", {
			BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1),
			Text = state and "✓" or "", TextColor3 = theme.AccentText,
			Font = theme.FontBold, TextSize = 15, Parent = switch,
		})
	else
		switch = New("Frame", {
			...
			AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, 0, 0.5, 0),
			Size = UDim2.fromOffset(38, 20), Parent = controlArea,
		}, { Util.Corner(10) })
		knob = New("Frame", {
	...
	function obj:SetDisabled(disabled, reason)
		obj._disabled = disabled; obj._disabledReason = reason
...
local function CreateInput(parent, theme, opts)
	...
	local box = New("Frame", {
		...
		AnchorPoint = Vector2.new(1, 0.5),
		Position = UDim2.new(1, 0, 0.5, 0),
		Size = UDim2.new(1, 0, 0, 30), Parent = controlArea,
	...
	function obj:Set(value) input.Text = tostring(value); if opts.Flag then CoreUI.Flags[opts.Flag] = value end end
	function obj:Get() return input.Text end
	track(input.FocusLost:Connect(function(enterPressed)
		local text = input.Text
		if opts.Numeric then
			local n = tonumber(text)
			if not n then input.Text = tostring(opts.Default or 0); return end
...
local function CreateSlider(parent, theme, opts)
	...
	local valueLabel = New("TextLabel", {
		...
		Position = UDim2.new(1, 0, 0.5, 0), Size = UDim2.new(1, 0, 1, 0),
		Text = string.format("%." .. rounding .. "f%s", default, suffix),
		Font = theme.Font, TextSize = 13, TextColor3 = theme.SubText,
		TextXAlignment = Enum.TextXAlignment.Right, Parent = controlArea,
	})
	...
	local knob = New("Frame", {
		...
		Size = UDim2.fromOffset(14, 14), ZIndex = 2, Parent = rail,
	...
	local hit = New("TextButton", {
		...
		Position = UDim2.new(0, 0, 0, -7), Parent = rail,
	})
	local obj = { Instance = row, Flag = opts.Flag }
...
local function CreateDropdownBase(parent, theme, opts, multi)
	...
	local label = New("TextLabel", {
		...
		Size = UDim2.new(1, -30, 1, 0), Text = summaryText(),
		Font = theme.Font, TextSize = 13, TextColor3 = theme.Text,
	...
	New("TextLabel", {
		...
		Text = "▾", TextColor3 = theme.SubText, Font = theme.Font,
		TextSize = 13, Parent = box,
	})
	local popup = New("Frame", {
		BackgroundColor3 = theme.Card, Visible = false, ZIndex = 500,
		Size = UDim2.new(0, 0, 0, 0), Parent = ScreenGui,
	}, { Util.Corner(6), Util.Stroke(theme.Border, 1), Util.Pad(4, 4, 4, 4) })
	local list = New("ScrollingFrame", {
		BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1),
		CanvasSize = UDim2.new(), AutomaticCanvasSize = Enum.AutomaticSize.Y,
		ScrollBarThickness = 3, ZIndex = 500, Parent = popup,
	})
	New("UIListLayout", { Parent = list, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 2) })
	local optionButtons = {}
	local function isSelected(name)
		if multi then return table.find(selected, name) ~= nil end
	...
	function obj:Refresh(newOptions)
		...
		for i, name in ipairs(options) do
			local optBtn = New("TextButton", {
				...
				AutoButtonColor = false, ZIndex = 500, LayoutOrder = i, Parent = list,
			}, { Util.Corner(4) })
	...
	obj:Refresh(options)
	local open = false
	closePopup = function() open = false; popup.Visible = false end
	openPopup = function()
		if obj._disabled then return end
		open = true
...
local function CreateKeybind(parent, theme, opts)
	...
	local obj = { Instance = row, Flag = opts.Flag }
	track(btn.MouseButton1Click:Connect(function()
		listening = true; btn.Text = "..."; btn.TextColor3 = theme.Accent
	end))
	track(UserInputService.InputBegan:Connect(function(input, gpe)
		if listening then
			if input.UserInputType == Enum.UserInputType.Keyboard then
				currentKey = input.KeyCode
				btn.Text = currentKey.Name; btn.TextColor3 = theme.SubText
				listening = false
	...
	function obj:Set(keyCode) currentKey = keyCode; btn.Text = keyCode and keyCode.Name or "None" end
...
local function CreateColorPicker(parent, theme, opts)
	...
	local row, controlArea = BuildRow(parent, theme, {
		Title = opts.Title, Description = opts.Description, Icon = opts.Icon, ControlWidth = 44,
	})
	local swatch = New("TextButton", {
		BackgroundColor3 = color,
		AnchorPoint = Vector2.new(1, 0.5),
		Position = UDim2.new(1, 0, 0.5, 0),
		Size = UDim2.fromOffset(36, 28), Text = "",
		AutoButtonColor = false, Parent = controlArea,
	}, { Util.Corner(6), Util.Stroke(theme.Border, 1) })
	local popup = New("Frame", {
		BackgroundColor3 = theme.Card, Visible = false, ZIndex = 500,
		Size = UDim2.fromOffset(220, 190), Parent = ScreenGui,
	}, { Util.Corner(8), Util.Stroke(theme.Border, 1), Util.Pad(10, 10, 10, 10) })
	local svBox = New("Frame", {
		BackgroundColor3 = Color3.fromHSV(h, 1, 1),
		Size = UDim2.new(1, 0, 0, 110), ZIndex = 500, Parent = popup,
	}, { Util.Corner(6) })
	local whiteGrad = New("Frame", {
		BackgroundColor3 = Color3.new(1, 1, 1),
		Size = UDim2.fromScale(1, 1), ZIndex = 500, Parent = svBox,
	}, { Util.Corner(6), New("UIGradient", { Transparency = NumberSequence.new({
		NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 1),
	}) }) })
	New("Frame", {
		BackgroundColor3 = Color3.new(0, 0, 0),
		Size = UDim2.fromScale(1, 1), ZIndex = 500, Parent = whiteGrad,
	}, { Util.Corner(6), New("UIGradient", {
	...
	local svCursor = New("Frame", {
		...
		Position = UDim2.new(s, 0, 1 - v, 0), ZIndex = 501, Parent = svBox,
	}, { Util.Corner(5), Util.Stroke(Color3.new(0, 0, 0), 1) })
	local hueBar = New("Frame", {
		Size = UDim2.new(1, 0, 0, 14), Position = UDim2.new(0, 0, 0, 118),
		ZIndex = 500, Parent = popup,
	}, { Util.Corner(6), New("UIGradient", { Color = ColorSequence.new({
	...
	local hueCursor = New("Frame", {
		...
		Position = UDim2.new(h, 0, 0.5, 0), ZIndex = 501, Parent = hueBar,
	...
	local hexBox = New("TextBox", {
		...
		Font = theme.Font, TextSize = 13, TextColor3 = theme.Text,
		ClearTextOnFocus = false, ZIndex = 500, Parent = popup,
	}, { Util.Corner(6), Util.Stroke(theme.Border, 1) })
	local obj = { Instance = row, Flag = opts.Flag }
	...
	track(UserInputService.InputBegan:Connect(function(input)
		if open and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
			...
			if not inPopup and not inSwatch then open = false; popup.Visible = false end
		end
	end))
...
local function NewGroupbox(parent, theme, title, description)
	...
	if title then
		...
		New("TextLabel", {
			...
			TextXAlignment = Enum.TextXAlignment.Left,
			Size = UDim2.new(1, 0, 0, 22), Parent = header,
		})
		if description then
			New("TextLabel", {
				...
				AutomaticSize = Enum.AutomaticSize.Y,
				Size = UDim2.new(1, 0, 0, 0), Parent = header,
			})
		end
	end
...
local function nextOrder(self) self._order = self._order + 1; return self._order end

function Groupbox:AddLabel(text) local o = CreateLabel(self.Instance, self.Theme, text); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddDivider() local d = CreateDivider(self.Instance, self.Theme); d.LayoutOrder = nextOrder(self); return { Instance = d } end
function Groupbox:AddWarningBox(opts) local o = CreateWarningBox(self.Instance, self.Theme, opts); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddButton(opts) local o = CreateButton(self.Instance, self.Theme, opts); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddToggle(opts) local o = CreateToggleBase(self.Instance, self.Theme, opts, false); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddCheckbox(opts) local o = CreateToggleBase(self.Instance, self.Theme, opts, true); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddInput(opts) local o = CreateInput(self.Instance, self.Theme, opts); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddSlider(opts) local o = CreateSlider(self.Instance, self.Theme, opts); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddDropdown(opts) local o = CreateDropdownBase(self.Instance, self.Theme, opts, false); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddMultiDropdown(opts) local o = CreateDropdownBase(self.Instance, self.Theme, opts, true); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddKeybind(opts) local o = CreateKeybind(self.Instance, self.Theme, opts); o.Instance.LayoutOrder = nextOrder(self); return o end
function Groupbox:AddColorPicker(opts) local o = CreateColorPicker(self.Instance, self.Theme, opts); o.Instance.LayoutOrder = nextOrder(self); return o end
...
local function NewTabbox(parent, theme)
	...
	local pages = New("Frame", {
		BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y,
		Size = UDim2.new(1, 0, 0, 0), LayoutOrder = 1, Parent = outer,
	})
	...
	function tabbox:AddTab(name)
		local pill = New("TextButton", {
			...
			Text = "  " .. name .. "  ",
			Font = theme.Font, TextSize = 13, TextColor3 = theme.SubText,
			AutoButtonColor = false, Parent = pillBar,
		}, { Util.Corner(6) })
		local page = New("Frame", {
			BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y,
			Size = UDim2.new(1, 0, 0, 0), Visible = false, Parent = pages,
		})
...
function CoreUI:CreateWindow(config)
	...
	New("Frame", {
		...
		Position = UDim2.new(0, 0, 1, -1), Parent = topbar,
	})
	if config.Icon then
	...
	local closeBtn = New("TextButton", {
		...
		Font = theme.FontBold, TextSize = 15, Parent = topbar,
	})
	...
	-- Sidebar: outer ScrollingFrame with padding, inner Frame with UIListLayout.
	-- This is the fix for "tabs appear far down".
	...
	New("Frame", {
		...
		Position = UDim2.new(1, 0, 0, 0), Parent = sidebar,
	})
	...
	local grip = New("Frame", {
		...
		Size = UDim2.fromOffset(18, 18), ZIndex = 50, Parent = main,
	})
	New("TextLabel", {
		BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1),
		Text = "◢", TextColor3 = theme.MutedText,
		Font = theme.Font, TextSize = 14, Parent = grip,
	})
	...
	local win = setmetatable({
		Instance = main, Theme = theme,
		Sidebar = sidebarInner, ContentHolder = contentHolder,
		_tabs = {}, _activeTab = nil, Visible = true,
	}, Window)
	...
	track(minimizeBtn.MouseButton1Click:Connect(function()
		...
		Util.Tween(main, 0.18, {
			Size = minimized and UDim2.fromOffset(size.X, 52)
				or UDim2.fromOffset(main.AbsoluteSize.X, size.Y)
		})
	...
	if Util.IsMobile() then
		local bubble = New("TextButton", {
			...
			Text = "☰", TextColor3 = theme.AccentText,
			Font = theme.FontBold, TextSize = 20,
			ZIndex = 9000, Parent = ScreenGui,
		}, { Util.Corner(24) })
		Util.Draggify(bubble, bubble)
		track(bubble.MouseButton1Click:Connect(function() win:SetVisible(not win.Visible) end))
	end

	function win:SetVisible(v) self.Visible = v; main.Visible = v end
	function win:SetStatus(text) statusText.Text = text or "" end
	...
	function win:CreateTab(name, iconName)
		...
		local sideLabel = New("TextLabel", {
			...
			Text = name, Font = theme.Font, TextSize = 14,
			TextColor3 = theme.SubText,
			TextXAlignment = Enum.TextXAlignment.Left, Parent = sideBtn,
		})

		local tabObj = setmetatable({
			Page = page, Columns = columns, Theme = theme,
			_leftCol = nil, _rightCol = nil,
		}, Tab)
...
function Tab:_getSplit()
	if not self._split then
		self._split = New("Frame", {
			BackgroundTransparency = 1,
			Size = UDim2.new(1, 0, 0, 0),
			AutomaticSize = Enum.AutomaticSize.Y, Parent = self.Columns,
		})
		New("UIListLayout", {
...
function Tab:CreateTabbox() return NewTabbox(self.Columns, self.Theme) end
function Tab:CreateWarningBox(opts) return CreateWarningBox(self.Columns, self.Theme, opts) end
...
function CoreUI:ShowKeybindMenu()
	...
	New("TextLabel", {
		...
		TextXAlignment = Enum.TextXAlignment.Left,
		Size = UDim2.new(1, 0, 0, 20), Parent = menu,
	})
	for _, entry in ipairs(self._KeybindElements) do
		...
		New("TextLabel", {
			BackgroundTransparency = 1, Text = entry.Title,
...
-- Try to load Lucide automatically. If it works, icons appear.
-- If not, the UI still works without icons.
task.spawn(function()
	Icons.AutoLoad()
	CoreUI._iconsReady = Icons._loaded
end)
